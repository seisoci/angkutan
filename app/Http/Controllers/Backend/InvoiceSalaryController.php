...
class InvoiceSalaryController extends Controller
{
      ...
      try {
        ...
        if (($checksaldo->saldo ?? FALSE) && $request->grand_total <= $checksaldo->saldo) {
          ...
          $response = response()->json([
            ...
            'redirect' => route('backend.invoicesalaries.index'),
          ]);
        } else {
          DB::rollBack();
          $response = response()->json([
  ...
  public function create(Request $request)
  {
    ...
    $selectCoa = ConfigCoa::with('coa')
      ->where('name_page', 'invoicesalaries')
      ->sole();
