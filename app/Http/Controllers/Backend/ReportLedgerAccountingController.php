...
class ReportLedgerAccountingController extends Controller
{
  use CarbonTrait;

  function __construct()
  {
    $this->middleware('permission:ledgeraccounting-list|ledgeraccounting-create|ledgeraccounting-edit|ledgeraccounting-delete', ['only' => ['index']]);
  }
  ...
  public function index(Request $request)
  {
    ...
    return view('backend.report.reportledgeraccounting.index', compact('config', 'page_breadcrumbs', 'data', 'date'));
  }
  ...
  public function document(Request $request)
  {
    ...
    $sheet->setCellValue('A1', 'Laporan Buku Besar Akunting');
    ...
    foreach ($data as $keyCollection => $itemCollection):
      ...
      endforeach;
    endforeach;
    $filename = 'Laporan Buku Besar Akunting ' . $this->dateTimeNow();
    if ($type == 'EXCEL') {
