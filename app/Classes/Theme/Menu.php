...
class Menu
{
  ...
  public static function DbrolePermissions()
  {
  ...
  public static function renderVerMenu($item, $parent = null, $rec = 0, $singleItem = false, $permission = array(), $role = NULL)
  {
    ...
    if ($role == 'super-admin') {
      ...
      $dataSplit = NULL;
      if (isset($item['page'])) {
        $split = explode("/", $item['page']);
        $dataSplit = $split[1];
      }
      $hidden = NULL;
      if (!in_array($dataSplit, $permission)) {
        $hidden = 'hidden';
        if (isset($item['list'])) {
          if (array_intersect($permission, $item['list'])) {
            $hidden = NULL;
          }
        }
      }
    ...
    if (isset($item['separator'])) {
      ...
      echo '<li class="menu-item ' . $item_class . '" aria-haspopup="true" ' . $item_attr . $hidden . '>';
      if (isset($item['parent'])) {
      ...
      if (isset($item['submenu'])) {
        ...
        if (isset($item['root'])) {
          ...
          self::renderVerMenu($parent_item, null, $rec++, true, $permission); // single item render
        }
        foreach ($item['submenu'] as $submenu_item) {
          self::renderVerMenu($submenu_item, $item, $rec++, false, $permission, $role);
        }
        echo '</ul>';
      ...
      foreach ($item['items'] as $each) {
        self::renderVerMenu($each, $parent, $rec++, NULL, $item['permissions'], $item['role']);
      }
    }
  }
  ...
  public
  static function renderHorMenu($item, $parent = null, $rec = 0)
  {
...
// Check for active Vertical Menu item
  public
  static function isActiveVerMenuItem($item, $page, $rec = 0)
  {
...
// Check for active Horizontal Menu item
  public
  static function isActiveHorMenuItem($item, $page, $rec = 0)
  {
    if (isset($item['redirect']) && $item['redirect'] == $page) {
      return false;
    }
...
// Checks recursion depth
  public
  static function checkRecursion($rec, $max = 10000)
  {
    if ($rec > $max) {
