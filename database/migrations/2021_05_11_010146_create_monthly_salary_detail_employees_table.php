...
class CreateMonthlySalaryDetailEmployeesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('monthly_salary_detail_employees', function (Blueprint $table) {
            $table->id();
            $table->foreignId('monthly_salary_detail_id')->references('id')->on('monthly_salary_details')->cascadeOnDelete();
            $table->foreignId('employee_master_id')->references('id')->on('employee_masters')->cascadeOnUpdate();
