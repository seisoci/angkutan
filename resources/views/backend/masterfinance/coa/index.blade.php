        ...
        @foreach($collection as $keyCollection => $itemCollection)
          ...
          @foreach ($itemCollection as $itemParent)
            <tr class="table-dark font-weight-bold text-dark">
              ...
              <td><a href="#" data-toggle="modal" data-target="#modalEdit" data-id="{{ $itemParent->id }}"
                     data-name="{{ $itemParent->name }}" class="edit btn btn-warning btn-sm">Edit</a>
              {!! $itemParent->can_delete == '1' ? '<a href="#" data-toggle="modal" data-target="#modalDelete" data-id="'.$itemParent->id.'" class="delete btn btn-danger btn-sm">Delete</a>' : NULL !!}
            ...
            @foreach($itemParent->children as $itemChildren)
              <tr class="font-weight-bold">
                ...
                <td><a href="#" data-toggle="modal" data-target="#modalEdit" data-id="{{ $itemChildren->id }}"
                       data-name="{{ $itemChildren->name }}" class="edit btn btn-warning btn-sm">Edit</a>
                  {!! $itemChildren->can_delete == '1' ? '<a href="#" data-toggle="modal" data-target="#modalDelete" data-id="'.$itemChildren->id.'" class="delete btn btn-danger btn-sm">Delete</a>' : NULL !!}
                </td>
              </tr>
            @endforeach
          @endforeach
        ...
        <form id="formUpdate" action="#">
          ...
          <div class="modal-body">
            ...
            <div class="form-group">
              ...
              <input type="text" name="name" class="form-control form-control-solid" placeholder="Input Nama Brand"/>
            </div>
          </div>
  ...
  <script type="text/javascript">
    $(document).ready(function () {
      ...
      $('#modalEdit').on('show.bs.modal', function (event) {
        ...
        $(this).find('#formUpdate').attr('action', '{{ route("backend.mastercoa.index") }}/' + id)
      ...
      $('#modalDelete').on('show.bs.modal', function (event) {
        let id = $(event.relatedTarget).data('id');
        $(this).find('.modal-body').find('a[name="id"]').attr('href', '{{ route("backend.mastercoa.index") }}/' + id);
      });
