...
<div class="row">
  ...
  <div class="col-sm-6">
    <form id="formUpdate" action="{{ route('backend.menus.update', Request::segment(3)) }}" autocomplete="off">
      <meta name="csrf-token" content="{{ csrf_token() }}">
      @method('PUT')
      ...
      <div class="card card-table">
        ...
        <div class="card-body">
          <div class="form-group">
            ...
            <div style="position:relative; display:block;">
              <input id="in_title" type="text" class="form-control input-sm" name="title" value="{{ $menu->title }}" />
              <div id="result"></div>
            </div>
          </div>
          <div class="form-group">
            <label>Url</label>
            <input id="in_url" type="text" class="form-control input-sm" name="url" value="{{ $menu->url }}" />
          </div>
          <div class="form-group">
            <label style="display: block;">Target</label>
            <div class="btn-group btn-group-toggle" data-toggle="buttons">
              <label class="btn btn-sm btn-info {{ $menu->target == '_self' ? 'active' : NULL }}">
                <input type="radio" name="target" value="_self" {{ $menu->target == '_self' ? 'checked' : NULL }}> Self
              </label>
              <label class="btn btn-sm btn-info {{ $menu->target == '_self' ? 'active' : NULL }}">
                <input type="radio" name="target" value="_blank" {{ $menu->target == '_blank' ? 'checked' : NULL }}>
                Blank
              </label>
            </div>
          </div>
        ...
        <div class="card-footer">
          <div class="btn-group"><a href="javascript:history.back()" class="btn btn-sm btn-info text-white"><i
                class="fa fa-fw fa-reply"></i>
              Back</a>
            <button type="submit" class="btn btn-sm btn-success text-white">Edit <i class="fa fa-fw fa-save"></i></div>
        </div>
      </div>
    </form>
  </div>
